...
module RV32IM72F8SP #(
    ...
    input reset,
    input UART_busy,
    input [XLEN-1:0] mmio_kb_data,
    input [XLEN-1:0] mmio_kb_status,
    input timer_interrupt_pending,
    input [XLEN-1:0] clint_rdata,
    ...
    reg [2:0] mmio_read_sel;  // 0=none, 1=UART status, 2=KB data, 3=KB status
 
    always @(posedge clk or posedge reset) begin
        if (reset)
            mmio_read_sel <= 3'd0;
        else if (clk_enable && !EX_MEM_stall) begin
            casez (EX2_alu_result)
                32'h1001_0004: mmio_read_sel <= 3'd1;   // UART status
                32'h1003_0000: mmio_read_sel <= 3'd2;   // Keyboard scan code
                32'h1003_0004: mmio_read_sel <= 3'd3;   // Keyboard status
                32'h0200_0???: mmio_read_sel <= 3'd4;   // CLINT
                default:       mmio_read_sel <= 3'd0;   // Normal memory
            endcase
        end
    end
    ...
    assign data_memory_read_data_muxed =
        (mmio_read_sel == 3'd1) ? {31'b0, UART_busy}   :
        (mmio_read_sel == 3'd2) ? mmio_kb_data         :
        (mmio_read_sel == 3'd3) ? mmio_kb_status       :
        (mmio_read_sel == 3'd4) ? clint_rdata          :
