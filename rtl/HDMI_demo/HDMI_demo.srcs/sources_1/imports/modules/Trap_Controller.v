        ...
        default: begin
            case (trap_handle_state)
                IDLE: begin 
                    ...
                    else if (trap_status == `TRAP_ECALL || trap_status == `TRAP_TIMER_IRQ) begin
                        standby_mode = 1'b1;
                        trap_done = 1'b0;
                ...
                WRITE_MEPC: begin 
                    ...
                    else if (trap_status == `TRAP_TIMER_IRQ) csr_trap_write_data = 32'h8000_0007;
                ...
                READ_MEPC: begin
                    ...
                    csr_trap_address = 12'h341; // mepc
                    // FreeRTOS fix: MRET은 정확히 mepc로 복귀해야 함.
                    // +4는 소프트웨어(portASM.S)가 ECALL일 때만 적용.
                    // 기존 +4는 timer interrupt 복귀 시 명령어 skip을 유발.
                    trap_target = {csr_read_data[XLEN-1:2], 2'b0};
                    trap_done = 1'b0;
